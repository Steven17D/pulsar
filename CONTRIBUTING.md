...
Pulsar also talks to macOS audio services and real WLED devices, so many
changes still need manual verification. For UI, audio capture, device
discovery, DDP output, or packaging changes, **manually exercise the
affected code path against at least one real WLED device** and state
which device(s) in the PR description.

Keep new unit tests under `Tests/PulsarTests/` so SwiftPM and CI pick
them up automatically. Good test candidates are pure logic: config
sanitization, DDP packet framing, palette interpolation, and segment
mirror/reverse serialization.
