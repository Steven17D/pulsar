...
final class Mapper {
    ...
    static func pretty(_ id: String) -> String {
        ...
        case "solid":      return "Solid · Palette"
    ...
    func serialize(into out: inout [UInt8]) {
        ...
        for p in wire {
            out.append(scale(p.r)); out.append(scale(p.g)); out.append(scale(p.b))
            if rgbw { out.append(scale(p.w)) }
        }
    }
    ...
    /// brightness modulated by smoothed power × intensity.
    private func renderSolid(power: Float, dt: Float) {
        ...
        let v = max(0.05, smoothPower)
        phase += dt * 0.08
        if phase > 1 { phase -= 1 }
        let c = palette.sample(at: phase)
    ...
    private func renderWavelength(bands: [Float], dt: Float) {
        ...
        for i in 0..<renderLen {
            ...
            let c = palette.sample(at: t < 0 ? t + 1 : t)
